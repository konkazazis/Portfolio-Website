        ...
        <div class="flex flex-wrap gap-2">
            @foreach ($this->categories as $cat)
                <button type="button" wire:click="setCategory('{{ $cat }}')"
                    ...
                    {{ $cat === 'all' ? 'All' : ucfirst(str_replace('-', ' ', $cat)) }}
                </button>
            @endforeach
        </div>
        ...
        @forelse ($this->posts as $post)
            <article class="post group grid items-start gap-[clamp(16px,3vw,40px)] grid-cols-1 md:grid-cols-[150px_1fr] py-9 {{ ! $loop->last ? 'border-b border-line' : '' }}">
                <div class="flex md:flex-col items-center md:items-start gap-3 md:gap-2 pt-1">
                    ...
                    <span class="font-mono text-[12px] text-muted">{{ $post->published_at?->format('M Y') }}</span>
                </div>
                <div>
                    <h2 class="text-[clamp(1.4rem,2.8vw,1.9rem)] tracking-[-0.025em] font-bold leading-[1.12]">
                        <a href="{{ route('show', $post->slug) }}" class="group-hover:text-accent-ink transition">{{ $post->title }}</a>
                    </h2>
                    ...
                    <div class="flex items-center justify-between flex-wrap gap-4 mt-6">
                        ...
                        <a href="{{ route('show', $post->slug) }}" class="inline-flex items-center gap-2 font-semibold text-[0.92rem] border-b-[1.5px] border-line pb-[2px] transition-all group-hover:border-accent group-hover:gap-3">
