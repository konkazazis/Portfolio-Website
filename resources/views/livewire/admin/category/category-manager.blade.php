<div class="p-6 lg:p-10">
    ...
    <div class="flex items-start justify-between mb-8">
        <div>
            ...
            <p class="text-zinc-500 dark:text-zinc-400 text-sm mt-1">Manage post categories.</p>
        </div>
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-sm overflow-hidden">
        @if($categories->isEmpty())
            <div class="py-16 text-center text-zinc-400 dark:text-zinc-500 text-sm">No categories found.</div>
        @else
                ...
                <tbody class="divide-y divide-zinc-50 dark:divide-zinc-800">
                    @foreach($categories as $category)
                        <tr class="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-colors">
                            ...
                            <td class="px-4 py-4 text-zinc-400 dark:text-zinc-500 text-xs font-mono hidden sm:table-cell">{{ $category->slug }}</td>
                            <td class="px-4 py-4 text-zinc-600 dark:text-zinc-300 hidden sm:table-cell">{{ $category->posts_count }}</td>
                            <td class="px-6 py-4">
            ...
            @if($categories->hasPages())
                <div class="px-6 py-4 border-t border-zinc-100 dark:border-zinc-800">{{ $categories->links() }}</div>
            @endif
        @endif
    </div>
    ...
    <div x-data="{ show: @entangle('deletingId').live }"
        ...
        <div @click.stop class="bg-white dark:bg-zinc-900 rounded-2xl shadow-xl w-full max-w-sm p-6 text-center border border-zinc-100 dark:border-zinc-800">
            <p class="text-lg font-bold text-zinc-800 dark:text-zinc-100 mb-2">Delete category?</p>
            <p class="text-sm text-zinc-500 mb-6">Posts in this category will be uncategorised.</p>
